...
namespace error_system::module {
    ...
    enum class common_module_t : uint16_t {
        ...
        throttler = 0x0815,        // 限流器
        retryer = 0x0816,          // 重试器
        circuit_breaker = 0x0817,  // 熔断器
        health_checker = 0x0818,   // 健康检查器
        discovery = 0x0819,        // 服务发现
        registry = 0x081A,         // 服务注册
        config_loader = 0x081B,    // 配置加载器
        event_bus = 0x081C,        // 事件总线
        command_bus = 0x081D,      // 命令总线
        query_bus = 0x081E,        // 查询总线
        saga = 0x081F,             //  Saga协调器
        outbox = 0x0820,           // 发件箱模式
        inbox = 0x0821,            // 收件箱模式
        idempotency = 0x0822,      // 幂等性控制
        audit = 0x0823,            // 审计记录器
        meter = 0x0824,            // 度量收集器
        tracer = 0x0825,           // 链路追踪器
        logger = 0x0826,           // 日志记录器
