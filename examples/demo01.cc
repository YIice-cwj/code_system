 ...
 *          每个示例独立成函数，标题即 API 名，便于快速查找。
 *          符号位约定：0=成功，非0=失败（Unix 退出码约定）。
 */
...
/** @brief 1.1 构造：错误码 + 消息（最常用） */
void demo_construction_code_message() {
    section("1.1 构造：错误码 + 消息（最常用）");
    error_context_t ctx{biz::trade_errors::ERR_ORDER_NOT_FOUND, "订单查询失败"};
    std::cout << "  " << ctx << std::endl;
}

/** @brief 1.2 构造：错误码 + 消息 + 源位置（located_code_t 自动捕获） */
void demo_construction_with_source_location() {
    section("1.2 构造：错误码 + 消息 + 源位置");
    error_context_t ctx{biz::trade_errors::ERR_ORDER_NOT_FOUND, "带源位置"};
    std::cout << "  " << ctx << std::endl;
}

/** @brief 1.3 构造：默认构造（sign=0=成功） */
void demo_construction_default() {
    section("1.3 构造：默认构造（sign=0=成功）");
    error_context_t default_ctx;
    std::cout << "  is_success=" << default_ctx.is_success()
              << " is_error=" << default_ctx.is_error() << std::endl;
}

/** @brief 1.4 构造：from_exception 从 std::exception 创建 */
void demo_construction_from_exception() {
...
/** @brief 1.5 错误码解析：sign/level/system/subsystem/module/number */
void demo_error_code_parsing() {
    section("1.5 构造：错误码解析（sign/level/system/subsystem/module/number）");
    auto code = biz::trade_errors::ERR_ORDER_NOT_FOUND;
    std::cout << "  sign=" << static_cast<int>(code.get_sign()) << " (0=成功,1=失败)" << std::endl;
    ...
    std::cout << "  is_error_code=" << code.is_error_code()
              << " is_success_code=" << code.is_success_code() << std::endl;
}

/** @brief 2.1 with(string,string) 字符串键值 */
void demo_payload_string_kv() {
...
/** @brief 2.2 with(const char*, const char*) C 字符串 */
void demo_payload_c_string() {
...
/** @brief 2.3 with(string_view, string_view) 视图 */
void demo_payload_string_view() {
...
/** @brief 2.4 with(string&&, string&&) 移动语义 */
void demo_payload_move() {
...
/** @brief 2.5 with(key, T) 多类型（int/double/bool） */
void demo_payload_multi_type() {
...
/** @brief 2.6 with_batch 批量添加 */
void demo_payload_batch() {
...
/** @brief 2.7 get_payload_value 按 key 取值 */
void demo_payload_get_value() {
...
/** @brief 2.8 payload_size 项数 */
void demo_payload_size() {
...
/** @brief 2.9 is_payload_empty 判空 */
void demo_payload_is_empty() {
    section("2.9 is_payload_empty 判空");
    error_context_t p1{biz::trade_errors::ERR_ORDER_NOT_FOUND, "payload 演示"};
    p1.with("user_id", "8848");
    error_context_t ctx1{biz::trade_errors::ERR_ORDER_NOT_FOUND, "无 payload"};
    std::cout << "  p1 empty = " << p1.is_payload_empty() << " (应为 0)" << std::endl;
    std::cout << "  ctx1 empty = " << ctx1.is_payload_empty() << " (应为 1)" << std::endl;
}

/** @brief 2.10 get_payload 获取全部副本 */
void demo_payload_get_all() {
...
/** @brief 2.11 for_each_payload 遍历 */
void demo_payload_for_each() {
...
/** @brief 3.1 is_success / is_error（sign 位） */
void demo_predicate_success_error() {
    section("3.1 is_success / is_error（sign 位）");
    error_context_t ctx1{biz::trade_errors::ERR_ORDER_NOT_FOUND, "错误上下文"};
    error_context_t default_ctx;
    std::cout << "  ctx1: is_success=" << ctx1.is_success()
              << " is_error=" << ctx1.is_error() << std::endl;
    std::cout << "  default_ctx: is_success=" << default_ctx.is_success()
              << " is_error=" << default_ctx.is_error() << std::endl;
}

/** @brief 3.2 is_fatal 是否 fatal 级别 */
void demo_predicate_fatal() {
    section("3.2 is_fatal 是否 fatal 级别");
    error_context_t fatal_ctx{biz::payment_errors::ERR_ACCOUNT_FROZEN, "账户冻结"};
    error_context_t ctx1{biz::trade_errors::ERR_ORDER_NOT_FOUND, "非 fatal"};
    std::cout << "  fatal_ctx.is_fatal() = " << fatal_ctx.is_fatal() << std::endl;
    std::cout << "  ctx1.is_fatal() = " << ctx1.is_fatal() << std::endl;
}

/** @brief 3.3 is_retryable 是否可重试 */
void demo_predicate_retryable() {
    section("3.3 is_retryable 是否可重试");
    error_context_t ctx1{biz::trade_errors::ERR_ORDER_NOT_FOUND, "默认不可重试"};
    std::cout << "  ctx1.is_retryable() = " << ctx1.is_retryable() << std::endl;
    error_code_t retryable_code = biz::trade_errors::ERR_ORDER_NOT_FOUND;
    retryable_code.set_retryable(true);
    error_context_t retry_ctx{retryable_code, "标记为可重试"};
    std::cout << "  retry_ctx.is_retryable() = " << retry_ctx.is_retryable() << std::endl;
}

/** @brief 3.4 is_transient 是否瞬态 */
void demo_predicate_transient() {
    section("3.4 is_transient 是否瞬态");
    error_context_t ctx1{biz::trade_errors::ERR_ORDER_NOT_FOUND, "默认非瞬态"};
    std::cout << "  ctx1.is_transient() = " << ctx1.is_transient() << std::endl;
    error_code_t transient_code = biz::trade_errors::ERR_ORDER_NOT_FOUND;
    transient_code.set_transient(true);
    error_context_t trans_ctx{transient_code, "标记为瞬态"};
    std::cout << "  trans_ctx.is_transient() = " << trans_ctx.is_transient() << std::endl;
}

/** @brief 4.1 to_string 可读文本 */
void demo_serialize_to_string() {
...
/** @brief 4.2 to_json JSON 格式 */
void demo_serialize_to_json() {
...
/** @brief 4.3 to_binary 紧凑二进制 */
void demo_serialize_to_binary() {
...
/** @brief 5.1 operator== 完整比较（code+message+payload） */
void demo_compare_equal() {
...
/** @brief 5.3 equals_by_code 仅按错误码比较 */
void demo_compare_by_code() {
    ...
    std::cout << "  消息不同但码相同: " << same_code_a.equals_by_code(same_code_b)
              << " (应为 1)" << std::endl;
}

/** @brief 5.4 equals_strict 严格比较（含 cause/stack） */
void demo_compare_strict() {
    section("5.4 equals_strict 严格比较（含 cause/stack）");
    error_context_t eq_a{biz::trade_errors::ERR_ORDER_NOT_FOUND, "相同消息"};
    error_context_t eq_b{biz::trade_errors::ERR_ORDER_NOT_FOUND, "相同消息"};
    std::cout << "  eq_a.equals_strict(eq_b) = " << eq_a.equals_strict(eq_b)
              << " (应为 1)" << std::endl;
}

/** @brief 6.1 wrap(底层错误) 构建因果链 */
void demo_cause_chain_wrap_lvalue() {
...
/** @brief 6.2 wrap(右值错误) 移动语义版本 */
void demo_cause_chain_wrap_rvalue() {
...
int main() {
    ...
    demo_construction_code_message();
    demo_construction_with_source_location();
    demo_construction_default();
    demo_construction_from_exception();
    demo_error_code_parsing();

    demo_payload_string_kv();
    demo_payload_c_string();
    demo_payload_string_view();
    demo_payload_move();
    demo_payload_multi_type();
    demo_payload_batch();
    demo_payload_get_value();
    demo_payload_size();
    demo_payload_is_empty();
    demo_payload_get_all();
    demo_payload_for_each();

    demo_predicate_success_error();
    demo_predicate_fatal();
    demo_predicate_retryable();
    demo_predicate_transient();

    demo_serialize_to_string();
    demo_serialize_to_json();
    demo_serialize_to_binary();
    demo_what();
    demo_get_code();

    demo_compare_equal();
    demo_compare_not_equal();
    demo_compare_by_code();
    demo_compare_strict();

    demo_cause_chain_wrap_lvalue();
    demo_cause_chain_wrap_rvalue();
    demo_cause_chain_binary();
