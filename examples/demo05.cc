 ...
 *          多语言消息目录的全部公共 API。每个示例独立成函数，标题即 API 名。
...
/** @brief 构造一个自定义错误码（subsystem=150, module=1, number=N） */
...
/** @brief 1.1 register_error(code, name, desc) 单条注册 */
void demo_register_single() {
...
/** @brief 1.2 register_errors 批量注册（返回成功数量） */
void demo_register_batch() {
...
/** @brief 1.3 is_registered 是否已注册 */
void demo_is_registered() {
...
/** @brief 1.4 get_info 查询元数据（返回 optional 副本） */
void demo_get_info() {
...
/** @brief 1.5 get_info_cached 热路径缓存查询（thread_local） */
void demo_get_info_cached() {
...
/** @brief 1.6 invalidate_metadata_cache 清除当前线程缓存 */
void demo_invalidate_cache() {
...
/** @brief 1.7 get_epoch 注册表纪元（变更自增） */
void demo_get_epoch() {
...
/** @brief 2.1 find_by_name 按名称查找错误码 */
void demo_find_by_name() {
...
/** @brief 2.2 get_errors_by_module 按模块组查找 */
void demo_get_errors_by_module() {
...
/** @brief 2.3 get_errors_by_subsystem 按子系统查找 */
void demo_get_errors_by_subsystem() {
...
/** @brief 3.1 unregister_error(code) 按错误码注销 */
void demo_unregister_by_code() {
...
/** @brief 3.2 unregister_error(name) 按名称注销 */
void demo_unregister_by_name() {
...
/** @brief 3.3 unregister_module 注销整个模块组 */
void demo_unregister_module() {
...
/** @brief 4.1 get_duplicate_policy 默认策略（skip） */
void demo_duplicate_policy_default() {
    section("4.1 get_duplicate_policy 默认策略（skip）");
    auto& registry = error_registry_t::instance();
    std::cout << "  当前策略 = " << static_cast<int>(registry.get_duplicate_policy()) << std::endl;
}

/** @brief 4.2 set_duplicate_policy(skip) 重复时静默跳过 */
void demo_duplicate_policy_skip() {
...
/** @brief 4.3 set_duplicate_policy(overwrite) 重复时覆盖 */
void demo_duplicate_policy_overwrite() {
...
/** @brief 4.4 set_duplicate_policy(warn) + set_duplicate_warn_callback */
void demo_duplicate_policy_warn() {
...
/** @brief 4.5 get_duplicate_warn_callback 获取当前回调 */
void demo_get_warn_callback() {
...
/** @brief 5.1 to_string(locale) 枚举转字符串 */
void demo_locale_to_string() {
...
/** @brief 5.2 from_string(text) 字符串转枚举（未识别返回 en_US） */
void demo_locale_from_string() {
...
/** @brief 5.3 is_valid(text) 校验字符串 */
void demo_locale_is_valid() {
...
/** @brief 6.1 set_enable_i18n / is_i18n_enabled 总开关 */
void demo_i18n_enable() {
...
/** @brief 6.2 set_default_locale / get_default_locale 默认 locale */
void demo_default_locale() {
...
/** @brief 6.3 set_output_locale / get_output_locale 显式输出 locale */
void demo_output_locale() {
...
/** @brief 6.4 clear_output_locale 清除显式输出 locale */
void demo_clear_output_locale() {
...
/** @brief 6.5 resolve_output_locale 解析最终 locale（output→default） */
void demo_resolve_output_locale() {
...
/** @brief 7.1 register_subsystem_module(subsys, module, names) 默认 zh_CN */
void demo_catalog_register_default() {
...
/** @brief 7.2 register_subsystem_module(locale, subsys, module, names) 多语言 */
void demo_catalog_register_multilang() {
...
/** @brief 7.3 get_subsystem_module_info(locale, subsys, module) 指定 locale */
void demo_catalog_get_by_locale() {
...
/** @brief 7.4 get_subsystem_module_info(locale, fallback, subsys, module) 带回退 */
void demo_catalog_get_with_fallback() {
...
/** @brief 7.5 resolve_subsystem_module 接口形式（用于注入） */
void demo_catalog_resolve() {
...
/** @brief 7.6 clear_locale 清除指定 locale 的所有名称 */
void demo_catalog_clear_locale() {
...
/** @brief 7.7 clear 清空所有名称 */
void demo_catalog_clear() {
...
/** @brief 8.1 register_message(locale, code, message) 单条注册 */
void demo_msg_register_single() {
...
/** @brief 8.2 register_messages 批量注册 */
void demo_msg_register_batch() {
...
/** @brief 8.3 get_message(locale, code) 指定 locale 查询 */
void demo_msg_get_by_locale() {
...
/** @brief 8.4 get_message(code) 使用当前输出 locale */
void demo_msg_get_current() {
...
/** @brief 8.5 set_default_locale / get_default_locale 委托 i18n_config_t */
void demo_msg_default_locale() {
...
/** @brief 8.6 set_active_locale / get_active_locale / clear_active_locale */
void demo_msg_active_locale() {
...
/** @brief 8.7 get_locales 已注册 locale 列表 */
void demo_msg_get_locales() {
...
/** @brief 8.8 message_count 指定 locale 消息数 */
void demo_msg_count() {
...
/** @brief 8.9 clear_locale / clear_all 清除消息 */
void demo_msg_clear() {
...
/** @brief 9.1 i18n 开启 + 中文 locale（输出名称） */
void demo_i18n_output_zh() {
...
/** @brief 9.2 i18n 开启 + 英文 locale（切换输出语言） */
void demo_i18n_output_en() {
...
/** @brief 9.3 i18n 关闭（回退为原始 ID 数字） */
void demo_i18n_disabled() {
...
/** @brief 10.1 JSON 多语言注册 - zh_CN（交易服务/订单模块） */
void demo_json_multilang_zh() {
...
/** @brief 10.2 JSON 多语言注册 - en_US（Trade Service/Order Module） */
void demo_json_multilang_en() {
...
/** @brief 10.3 JSON 多语言注册 - ja_JP（取引サービス/注文モジュール） */
void demo_json_multilang_ja() {
...
int main() {
    ...
    demo_register_single();
    demo_register_batch();
    demo_is_registered();
    demo_get_info();
    demo_get_info_cached();
    demo_invalidate_cache();
    demo_get_epoch();

    demo_find_by_name();
    demo_get_errors_by_module();
    demo_get_errors_by_subsystem();

    demo_unregister_by_code();
    demo_unregister_by_name();
    demo_unregister_module();

    demo_duplicate_policy_default();
    demo_duplicate_policy_skip();
    demo_duplicate_policy_overwrite();
    demo_duplicate_policy_warn();
    demo_get_warn_callback();

    demo_locale_to_string();
    demo_locale_from_string();
    demo_locale_is_valid();

    demo_i18n_enable();
    demo_default_locale();
    demo_output_locale();
    demo_clear_output_locale();
    demo_resolve_output_locale();

    demo_catalog_register_default();
    demo_catalog_register_multilang();
    demo_catalog_get_by_locale();
    demo_catalog_get_with_fallback();
    demo_catalog_resolve();
    demo_catalog_clear_locale();
    demo_catalog_clear();

    demo_msg_register_single();
    demo_msg_register_batch();
    demo_msg_get_by_locale();
    demo_msg_get_current();
    demo_msg_default_locale();
    demo_msg_active_locale();
    demo_msg_get_locales();
    demo_msg_count();
    demo_msg_clear();
    ...
    demo_json_multilang_zh();
    demo_json_multilang_en();
    demo_json_multilang_ja();
